...
<style>
    ...
    * { font-family: 'Inter', sans-serif; box-sizing: border-box; }
    ...
    .rv-card-header.red    { background: #fef2f2; color: #dc2626; border-color: #fecaca; }
    ...
    .add-row-btn:hover { background: #fee2e2; border-color: #ef4444; transform: translateY(-1px); }
    ...
    .btn-save:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(220, 38, 38, 0.4); }
    .btn-outline-link {
...
<div class="rv-page">
    <div class="container-fluid" style="max-width: 1200px;">
        ...
        <div class="rv-header">
            <div>
                <h3><i class="bi bi-pencil-square me-2"></i>Edit Expense Voucher</h3>
                <p>Modify expense details. Upon saving, all ledger balances and transaction records will automatically synchronize.</p>
            </div>
            <div class="rv-badge">#{{ $voucher->evid }}</div>
        </div>
        ...
        <div class="rv-alert warning">
            ...
            <span><strong>ERP Ledger Sync:</strong> Updating this voucher will automatically reverse previous ledger postings for voucher #{{ $voucher->evid }} and re-post the new values accurately.</span>
        </div>

        <form action="{{ route('expense-vouchers.update', $voucher->id) }}" method="POST" id="expenseForm">
            @csrf
            @method('PUT')
            <input type="hidden" name="evid" value="{{ $voucher->evid }}">
                ...
                <div class="rv-card-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <input type="date" name="entry_date" class="form-control" value="{{ $voucher->entry_date }}" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Entry Date</label>
                            <input type="date" class="form-control" value="{{ $voucher->created_at ? $voucher->created_at->toDateString() : now()->toDateString() }}" readonly>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Voucher Remarks</label>
                            <input type="text" name="remarks" class="form-control" value="{{ $voucher->remarks }}" placeholder="e.g. Office maintenance / utility expense...">
                        </div>
                        @if($isSuperAdmin)
                        <div class="col-md-3">
            ...
            {{-- --- PAYMENT SOURCE (Paid From) --- --}}
            <div class="rv-card">
                <div class="rv-card-header red">
                    <i class="bi bi-wallet2"></i> Payment Source &mdash; Paid From Account / Party
                </div>
                <div class="rv-card-body">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="vendor_type" id="vendor_type" class="form-select" required>
                                ...
                                <option value="customer" data-kind="party" {{ $voucher->type === 'customer' ? 'selected' : '' }}>Customer</option>
                            </select>
                        </div>

                        {{-- Source Name --}}
                        <div class="col-md-4" id="party_name_container">
                            <label class="form-label" id="party_label">Select Source Account / Party <span class="text-danger">*</span></label>
                            <select name="vendor_id" id="vendor_id" class="form-select" required data-selected="{{ $voucher->party_id }}">
                                <option value="{{ $voucher->party_id }}" selected>{{ $partyName ?: 'Selected Source' }}</option>
                            </select>
                        </div>
            ...
            {{-- --- EXPENSE HEADS (Expense Rows) --- --}}
            <div class="rv-card">
                <div class="rv-card-header green">
                    <i class="bi bi-tag"></i> Expense Heads &mdash; Specific Expense Accounts
                    <span style="font-weight: 400; font-size: 0.78rem; margin-left: auto; opacity: 0.8;">You can split the expense across multiple expense heads</span>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="to-table" id="voucherTable">
                            <thead>
                                <tr>
                                    <th style="width:3%">#</th>
                                    <th style="width:29%">Narration</th>
                                ...
                                <tr class="voucher-row">
                                    ...
                                    <td>
                                        <div class="d-flex flex-nowrap gap-1">
                                            ...
                                            <button class="btn btn-outline-primary addNarrationBtn" type="button" title="Create New Narration" style="flex-shrink: 0; color: #dc2626; border-color: #fca5a5;">
            ...
            <div class="row g-3">
                    ...
                    <div class="rv-card">
                        ...
                        <div class="rv-card-body" style="padding: 1rem 1.5rem;">
                            <p style="margin:0; font-size: 0.85rem; color: #475569;">
                                Any modified amount or account will update General Ledger entries and expense accounts in real time.
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-md-5">
                    <div class="total-box mb-3">
                        <div class="total-label"><i class="bi bi-cash-stack me-1"></i> Total Expense Amount</div>
                    ...
                    <div class="d-flex flex-column gap-2">
                        ...
                        <a href="{{ route('all-expense-vochers') }}" class="btn-outline-link">
                            <i class="bi bi-arrow-left"></i> Cancel &amp; Back to Expenses
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-content" style="border-radius: 12px;">
      ...
      <div class="modal-body">
        <div class="mb-3">
          ...
          <input type="text" id="new_narration_text" class="form-control" placeholder="e.g. Electricity bill payment...">
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        <button type="button" class="btn btn-primary" id="saveNarrationModalBtn" style="background:#dc2626; border-color:#dc2626;">Save Narration</button>
      </div>
    </div>
  </div>
</div>
    ...
    function loadPartyList(selectedId) {
        ...
        $.ajax({
            ...
            success: function (data) {
                var options = '<option value="">— Select Source Account / Party —</option>';
