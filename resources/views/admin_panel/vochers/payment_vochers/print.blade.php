...
<head>
    ...
    <title>Payment Voucher - {{ $voucher->pvid }}</title>
    ...
    <style>
        :root {
            ...
            --accent: #6b0f8a; /* Custom Purple theme for Payment vouchers */
            --accent-light: #f3e8ff;
            --text-dark: #0f172a;
            --text-muted: #475569;
            --bg-light: #f8fafc;
...
<body>
    @php
        ...
        if (count($rows) === 1) {
            $senderName = $rows[0]['account_name'] ?? '-';
            $senderHead = $rows[0]['account_head'] ?? '-';
            $senderCode = $rows[0]['account_code'] ?? '-';
        } elseif (count($rows) > 1) {
            $senderName = 'Multiple Accounts';
            $senderHead = 'Mixed Heads';
            $senderCode = 'Mixed Codes';
        }
        ...
        if (is_numeric($voucher->type)) {
            ...
            $receiverHead = $party->head_name ?? 'Account Head';
            $receiverCode = $party->phone ?? '-';
            $receiverType = 'Account Head';
        } elseif ($voucher->type === 'vendor') {
            $receiverName = $party->name ?? '-';
            $receiverHead = 'Vendor Accounts';
            $receiverCode = $party->phone ?? '-';
            $receiverType = 'Vendor';
        } elseif (in_array($voucher->type, ['customer', 'walkin'])) {
            $receiverName = $party->customer_name ?? '-';
            $receiverHead = 'Customer Accounts';
            $receiverCode = $party->mobile ?? '-';
            $receiverType = $voucher->type === 'walkin' ? 'Walk-in Customer' : 'Customer';
        }
    @endphp
    ...
    <div class="page">
        ...
        <header>
            ...
            <div class="voucher-badge-container">
                ...
                <div>
                    <span class="receipt-badge" style="background-color: var(--primary);">PAYMENT VOUCHER</span>
                </div>
            </div>
        </header>
        ...
        <div class="meta-grid">
            <!-- Sender Details Card -->
            <div class="card">
                <div class="card-title">Sender / Paid From</div>
            ...
            <div class="card">
                <div class="card-title">Receiver / Paid To</div>
                ...
                @else
                    <div class="meta-line">
                        ...
                        <span class="meta-value" style="text-transform: capitalize; font-weight: 600;">{{ $receiverType }}</span>
                    </div>
                    <div class="meta-line">
                        <span class="meta-label">Phone:</span>
                        <span class="meta-value">{{ $receiverCode }}</span>
                    </div>
                @endif
            </div>
            ...
            <div class="card">
                ...
                <div class="meta-line">
                    ...
                    <span class="meta-value" style="font-weight: 800; color: var(--accent);">{{ $voucher->pvid }}</span>
                </div>
                <div class="meta-line">
        ...
        <div class="details-section">
            <h3 class="section-title">Itemized Payment Breakdowns</h3>
                ...
                <tbody>
                    ...
                    @php
                        ...
                        if (empty($rowNarration)) {
                            $rowNarration = "Payment Voucher — Paid to " . $receiverName;
                        }
                    @endphp
                    <tr>
                        <td>{{ $key + 1 }}</td>
        ...
        <div class="bottom-section">
            ...
            <div class="card words-card">
                <div class="words-title">Amount Paid in words</div>
