    ...
    <div class="voucher-form-container py-3">
        <div class="voucher-card">
            <div class="voucher-card-header d-flex align-items-center justify-content-between">
                <h6 class="mb-0 fs-6"><i class="fas fa-hand-holding-usd me-2"></i> Create Receipt Voucher</h6>
            ...
            <div class="card-body p-4">
                ...
                <form action="{{ route('inter_branch_vouchers.store_receipt') }}" method="POST">
                    ...
                    <!-- Section 1: Sending Branch & Account (Receipt From) -->
                    <div class="mb-4">
                        <div class="form-section-title">
                            <i class="fas fa-paper-plane"></i> 1. Sending Branch & Account (Receipt From)
                        </div>
                        <div class="row g-3">
                            <!-- Sending Branch -->
                            <div class="col-md-4">
                                <label class="form-label-compact">Received From Branch <span class="text-danger">*</span></label>
                    ...
                    <!-- Section 2: Receiving Branch & Account (Receipt To) -->
                    <div class="mb-4">
                        <div class="form-section-title">
                            <i class="fas fa-hand-holding-usd"></i> 2. Receiving Branch & Account (Receipt To)
                        </div>
                        <div class="row g-3">
                            <!-- Receiving Branch -->
                            <div class="col-md-4">
                                <label class="form-label-compact">Receiving Branch <span class="text-danger">*</span></label>
                                @if ($isSuperAdmin)
                                ...
                                @else
                                    @php
                                        $userBranch = $branches->firstWhere('id', $toBranchId);
                                    @endphp
                                    <input type="text" class="form-control form-control-compact bg-light fw-bold" readonly
                                        value="🏪 {{ $userBranch->name ?? $userBranch->branch_name ?? 'Branch #' . $toBranchId }}">
                                    <input type="hidden" name="to_branch_id" id="to_branch_id" value="{{ $toBranchId }}">
                                @endif
                                @error('to_branch_id')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="mb-4">
                        <div class="form-section-title">
                            <i class="fas fa-calculator"></i> 3. Receipt Amount & Notes
                        </div>
                        <div class="row g-3">
                            <div class="col-md-5">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="amount" class="form-control form-control-compact @error('amount') is-invalid @enderror"
                                        required min="0.01" step="0.01" placeholder="0.00"
                                        style="border-radius: 0 6px 6px 0 !important; font-weight: 700;">
                                </div>
                                @error('amount')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-7">
                                ...
                                <input type="text" name="remarks" class="form-control form-control-compact"
                    ...
                    <div class="d-flex gap-2 pt-2 border-top">
                        <button type="submit" class="btn btn-info text-white px-4 fw-bold" style="border-radius: 6px;">
                            <i class="fas fa-check-circle me-1"></i> Record Receipt Voucher
                        </button>
...
<script>
    $(document).ready(function() {
        ...
        // ==========================================
        // SENDING SIDE CASCADING LOGIC (From Branch)
        // ==========================================
        ...
        $('#from_head_id').on('change', function() {
            loadFromAccounts($('#from_branch_id').val(), $(this).val());
        });
        ...
        // ==========================================
        // RECEIVING SIDE CASCADING LOGIC (To Branch)
        // ==========================================
        ...
        $('#to_head_id').on('change', function() {
            loadToAccounts($('#to_branch_id').val(), $(this).val());
        });
        ...
        // Initial trigger: if To Branch is pre-set for regular branch users
        const initialToBranch = $('#to_branch_id').val();
        if (initialToBranch) {
            loadToHeads(initialToBranch);
        }
    });
</script>
@endsection
