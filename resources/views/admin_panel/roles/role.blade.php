                 ...
                 <div class="row">
                     <div class="col-lg-12">
                         <div class="d-flex justify-content-between align-items-center mb-3">
                             <h3>Roles</h3>
                             <div>
                                 @include('admin_panel.permissions.nav-links')
                             </div>
                             <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                 data-bs-target="#exampleModal" id="reset-form">Create</button>
                         </div>
                                                 ...
                                                 <tr>
                                                     ...
                                                     <td>
                                                         @forelse ($role->getPermissionNames() as $permission)
                                                             <span
                                                                 class="badge bg-success fw-bold p-2 text-white mb-2">{{ $permission }}</span>
                                                         @empty
                                                             <span class="badge bg-danger fw-bold p-2 text-white">No
                                                                 Permission Assigned</span>
                                                         @endforelse
                                                     </td>
                                                     <td>
                                                         ...
                                                         <a href="{{ route('roles.delete', $role->id) }}"
                                                             class="btn btn-danger btn-sm delete-btn p-1"
     ...
     <div class="modal fade" id="edit-permission-modal" tabindex="-1" aria-labelledby="exampleModalLabel"
         ...
         <div class="modal-dialog">
             <div class="modal-content">
                 ...
                 <form class="" action="{{ route('roles.update.permission') }}" method="POST">
                     <div class="modal-body">
                         @csrf
                         <input type="hidden" name="edit_id" id="edit-role-id" />
                         <div class="mb-3">
                             <label for="title" class="form-label">Name</label>
                             <input type="text" name="name" class="form-control" id="permission-modal-name"
                                 readonly />
                         </div>
                         {{-- <div class="mb-3">
                            <label for="title" class="form-label">Email</label>
                            <input type="text" name="email" class="form-control" id="permission-modal-email" readonly />
                        </div> --}}
                         <label for="title" class="form-label">Permissions</label>
                         <div class=" mb-3"  id="permission-checkbox-container">
         ...
         $(document).on('click', '.edit-permission-btn', function() {
             ...
             allPermissions.forEach(function(permission) {
                ...
                <div class="form-check d-flex  align-items-center mb-1" style="margin-left:140px;">
                    <input class="form-check-input me-2" type="checkbox" name="permissions[]" value="${permission.name}" ${isChecked}>
                    <label class="form-check-label pt-1">${permission.name}</label>
                </div>
            `);
             });
