...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('inward_gatepass_items', function (Blueprint $table) {
            if (!Schema::hasColumn('inward_gatepass_items', 'packing_type')) {
                $table->string('packing_type')->nullable()->after('qty');
            }
            if (!Schema::hasColumn('inward_gatepass_items', 'packing_qty')) {
                $table->decimal('packing_qty', 15, 2)->nullable()->after('packing_type');
            }
            if (!Schema::hasColumn('inward_gatepass_items', 'item_per_piece')) {
                $table->decimal('item_per_piece', 15, 2)->nullable()->after('packing_qty');
            }
            if (!Schema::hasColumn('inward_gatepass_items', 'loose_piece')) {
                $table->decimal('loose_piece', 15, 2)->nullable()->after('item_per_piece');
            }
            if (!Schema::hasColumn('inward_gatepass_items', 'unit')) {
                $table->string('unit')->nullable()->after('loose_piece');
            }
        });
    }
